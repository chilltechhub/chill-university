// src/components/CareerCompassGame.js
...
const BLURBS = {
  ...
  '3-5': 'Responsibility, goals, and teamwork.',
...
export default function CareerCompassGame({ onGameEnd }) {
  const navigation = useNavigation();
  const G = useGameTheme();
  const s = makeStyles(G);
  const { showEmojis } = useUIPrefs();
  const { level, setLevel, tier: savedTier } = useGradeLevel('career');
  ...
  const { next: nextFact } = useGameFacts('career');
  ...
  const beginRun = (selectedPace = 'relaxed') => {
    ...
    setAdaptive(initial);
    const first = pickNext(CAREER_BANK[levelForTier(initial.tier)], []);
  ...
  const loadNext = useCallback((tier) => {
        const pool = CAREER_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="career" showPace
        title="Career Compass" emoji="🧭" subjectLabel="Career & Life Skills"
  ...
  if (game.done) return (
    <GameOver gameId="career"
      score={game.score} correct={game.correct} total={game.attempted}
      streak={game.bestStreak} title="Career Ready!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="career" disableFactToast
      title="Career Compass" emoji="🧭" subject={`Career & Life Skills · ${levelForTier(adaptive.tier)}`}
