// src/components/PeopleSkillsGame.js
...
const BLURBS = {
  ...
  '3-5': 'Active listening and teamwork.',
...
export default function PeopleSkillsGame({ onGameEnd }) {
  const navigation = useNavigation();
  const G = useGameTheme();
  const s = makeStyles(G);
  const { showEmojis } = useUIPrefs();
  const { level, setLevel, tier: savedTier } = useGradeLevel('people');
  ...
  const loadNext = useCallback((tier) => {
        const pool = PEOPLE_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="people" showPace
        title="People Skills" emoji="🤝" subjectLabel="Social & Relationships"
  ...
  if (game.done) return (
    <GameOver gameId="people"
      score={game.score} correct={game.correct} total={game.attempted}
      streak={game.bestStreak} title="People Person!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="people" disableFactToast
      title="People Skills" emoji="🤝" subject={`Social & Relationships · ${levelForTier(adaptive.tier)}`}
