...
import { handleGameEvent } from '../logic/gamificationService';
...
export default function CoinGame() {
  const { user } = useUserProgress(); // assumes user.id exists
  ...
  const handleAnswer = async (userAnswer) => {
    ...
    const correct = userAnswer === hasEnough;
    const timeTaken = Math.floor((Date.now() - questionStartTime) / 1000);
   const { completeQuestion } = useUserProgress();
    ...
    // after correct is determined
    await completeQuestion('math', correct, 10, 5);
    ...
    if (correct) {
      ...
      setTimeout(() => setRetryMode(true), 2000);
    }
  };
  ...
  const handleRetrySubmit = async () => {
    ...
    // Emit retry QUESTION_ANSWERED event
    await handleGameEvent({
      type: 'QUESTION_ANSWERED',
      userId: user.id,
      gameId: 'coin_game',
    ...
    if (correct) {
      ...
      Alert.alert(
        'Try Again 🤔',
        `You counted ${counted}¢, but the actual total is ${difference}¢ ${counted > question.amount ? 'less' : 'more'}.`
      );
    }
  };
      ...
      <View style={styles.container}>
        ...
        {!retryMode ? (
          ...
          <View style={styles.retryContainer}>
            ...
            <Text style={styles.retryText}>Count each coin carefully and add them up.</Text>
            ...
            />
            <TouchableOpacity style={styles.submitButton} onPress={handleRetrySubmit}>
