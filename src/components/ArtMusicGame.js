// src/components/ArtMusicGame.js
...
import { ART_BANK } from '../data/gameContent/artMusic';

const BLURBS = {
  'K-2': 'Colors, instruments, and what art is.',
  '3-5': 'Tempo, sculpting, orchestras, and rhythm.',
  '6-8': 'Art movements, composers, and chords.',
  '9-12': 'Music theory and art history in depth.',
};
...
export default function ArtMusicGame({ onGameEnd }) {
  const navigation = useNavigation();
  const G = useGameTheme();
  const s = makeStyles(G);
  const { showEmojis } = useUIPrefs();
  const { level, setLevel, tier: savedTier } = useGradeLevel('art');
  ...
  const loadNext = useCallback((tier) => {
    const pool = ART_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="art" showPace
        title="Art & Music" emoji="🎨" subjectLabel="Art & Music"
  ...
  if (game.done) return (
    <GameOver gameId="art"
      ...
      streak={game.bestStreak} title="Art & Music Buff!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="art" disableFactToast
        title="Art & Music" emoji="🎨" subject={`Art & Music · ${levelForTier(adaptive.tier)}`}
