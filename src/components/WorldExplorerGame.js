// src/components/WorldExplorerGame.js
...
const BLURBS = {
  ...
  '3-5': 'Continents, the first President, and democracy basics.',
...
export default function WorldExplorerGame({ onGameEnd }) {
  const navigation = useNavigation();
  const G = useGameTheme();
  const s = makeStyles(G);
  const { showEmojis } = useUIPrefs();
  const { level, setLevel, tier: savedTier } = useGradeLevel('world');
  ...
  const loadNext = useCallback((tier) => {
        const pool = WORLD_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="world" showPace
        title="World Explorer" emoji="🌍" subjectLabel="Social Studies"
  ...
  if (game.done) return (
    <GameOver gameId="world"
      score={game.score} correct={game.correct} total={game.attempted}
      streak={game.bestStreak} title="World Explorer!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="world" disableFactToast
      title="World Explorer" emoji="🌍" subject={`Social Studies · ${levelForTier(adaptive.tier)}`}
