...
export default function TechLabGame({ onGameEnd }) {
  ...
  const { next: nextFact } = useGameFacts('tech');
  const [fact, setFact] = useState(null);
  // Relaxed pace: a fresh fact every few questions, not every one — see factCountRef below.
  const factCountRef = useRef(0);

  const game = useGame({ subject: 'technology', difficulty: adaptive.tier, skillLevel: level, onGameEnd, manualScoring: true });
  ...
  const loadNext = useCallback((tier) => {
        const pool = TECH_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="tech" showPace
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="tech" disableFactToast
