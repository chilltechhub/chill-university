// src/components/LingoMatchGame.js
...
import { LINGO_BANK } from '../data/gameContent/lingoMatch';

const BLURBS = {
  'K-2': 'Colors, animals, numbers — basic Spanish words.',
  '3-5': 'Family, food, and everyday phrases.',
  '6-8': 'Present-tense verb conjugation & question words.',
  '9-12': 'Past tense, idioms, and the subjunctive mood.',
};
...
export default function LingoMatchGame({ onGameEnd }) {
  const navigation = useNavigation();
  const G = useGameTheme();
  const s = makeStyles(G);
  const { showEmojis } = useUIPrefs();
  const { level, setLevel, tier: savedTier } = useGradeLevel('lingo');
  ...
  const loadNext = useCallback((tier) => {
        const pool = LINGO_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="lingo" showPace
        title="Lingo Match" emoji="🗣️" subjectLabel="Foreign Language"
  ...
  if (game.done) return (
    <GameOver gameId="lingo"
      score={game.score} correct={game.correct} total={game.attempted}
      streak={game.bestStreak} title="¡Excelente! Lingo Master!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="lingo" disableFactToast
      title="Lingo Match" emoji="🗣️" subject={`Foreign Language · ${levelForTier(adaptive.tier)}`}
