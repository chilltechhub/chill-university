// src/components/RegisterReadyGame.js
// Register Ready — front-of-house retail training quiz: cash handling, POS
// procedure, return judgment, and loss prevention. Structurally identical
// to CareerCompassGame.js (same quiz mechanic, adaptive difficulty, round
// structure) — only the content bank and the tier framing differ.
//
// Reuses the shared grade-band picker as an EXPERIENCE-tier picker, the
// same move surviveMonth.js already makes for "life stage" instead of
// school grade. See GradeSelectCard's `tierLabels`/`pickerPrompt` props.
...
const BLURBS = {
  'K-2': 'Greeting, honesty, and the basics every new hire starts with.',
  '3-5': 'Making change, clean returns, voids, and price checks.',
...
export default function RegisterReadyGame({ onGameEnd }) {
  ...
  const { next: nextFact } = useGameFacts('registerready');
  const [fact, setFact] = useState(null);
  const factCountRef = useRef(0);

  const game = useGame({ subject: 'career', difficulty: adaptive.tier, skillLevel: level, onGameEnd, manualScoring: true });

  const beginRun = (selectedPace = 'relaxed') => {
    setPace(selectedPace);
    const initial = createAdaptiveTier(savedTier);
    setAdaptive(initial);
    const first = pickNext(REGISTER_BANK[levelForTier(initial.tier)], []);
  ...
  const loadNext = useCallback((tier) => {
    const pool = REGISTER_BANK[levelForTier(tier)];
  ...
  if (!started) {
    return (
      <GradeSelectCard gameId="registerready" showPace
        title="Register Ready" emoji="🛒" subjectLabel="Retail Front-of-House"
        blurbs={BLURBS} tierLabels={TIER_LABELS} pickerPrompt="Choose your experience level"
  ...
  if (game.done) return (
    <GameOver gameId="registerready"
      score={game.score} correct={game.correct} total={game.attempted}
      streak={game.bestStreak} title="Register Ready!"
  ...
  if (roundComplete) {
    return (
      <GameShell gameId="registerready" disableFactToast
        title="Register Ready" emoji="🛒" subject={`Retail Front-of-House · ${TIER_LABELS[levelForTier(adaptive.tier)]}`}
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={s.scroll}>
        <Text style={s.progress}>Round {stage + 1} of {STAGE_COUNT} · Scenario {stageAsked + 1} of {stageTarget}</Text>
        ...
        {feedback && (
          <View style={[s.feedback, { borderColor: feedback.isCorrect ? G.success : G.error }]}>
            <Text style={[s.feedbackTitle, { color: feedback.isCorrect ? G.success : G.error }]}>
              {feedback.isCorrect ? '✓ Correct call' : `✗ Better move: "${feedback.correct}"`}
            </Text>
...
const makeStyles = (G) => StyleSheet.create({
  ...
  prompt:      { fontSize: 16, color: G.cream, textAlign: 'center', lineHeight: 23, fontWeight: '600' },
