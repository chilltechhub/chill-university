 ...
 * SelfCareScreen
 * ---------------------------------------
 * - Self-care categories (tabs)
 * - Search self-care actions
 * - Add custom actions
 * - Log self-care → popup reflection note
...
const DEFAULT_SELFCARE = [
  {
    ...
    items: [
      { id: 'b1', text: 'Showered' },
      { id: 'b2', text: 'Brushed Teeth' },
      { id: 'b3', text: 'Skin / Hair Care' }
    ]
  },
  {
    id: 'health',
    ...
    color: '#BBF7D0',
    items: [
      { id: 'h1', text: 'Took Medication' },
      { id: 'h2', text: 'Doctor / Therapy Task' },
      { id: 'h3', text: 'Hydrated Well' }
    ]
  },
  {
    id: 'life',
    ...
    color: '#FDE68A',
    items: [
      { id: 'l1', text: 'Cleaned Space' },
      { id: 'l2', text: 'Did Laundry' },
      { id: 'l3', text: 'Organized Something' }
    ]
  },
  {
    id: 'rest',
    ...
    items: [
      { id: 'r1', text: 'Rested' },
      { id: 'r2', text: 'Took Break' },
      { id: 'r3', text: 'Relaxed Intentionally' }
    ]
  }
];
...
const SelfCareScreen = ({ userId = null, supabaseClient = null }) => {

  const [categories, setCategories] = useState(DEFAULT_SELFCARE);
  const [selectedCategory, setSelectedCategory] = useState(DEFAULT_SELFCARE[0]);
  ...
  // ---------------------------------------
  // Load history & custom actions if Supabase exists
  // ---------------------------------------
  useEffect(() => {
    loadCustom();
    loadHistory();
  ...
  const handleLog = async (item) => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('selfcare_history').insert({
        user_id: userId,
        action: item.text,
        category: selectedCategory.id,
  ...
  const addAction = async () => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('custom_selfcare').insert({
        user_id: userId,
        action: newAction,
        created_at: new Date().toISOString()
      });
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Take care of yourself 💖</Text>
        </View>
        ...
        <View style={styles.addBox}>
          <Text style={styles.addTitle}>Add New Self Care</Text>
          <TextInput
            placeholder="ex: Took a walk"
            value={newAction}
        ...
        <View style={styles.journal}>
          <Text style={styles.journalTitle}>Weekly Self Care Journal</Text>
          ...
          {dayEntries.length === 0 && (
            <Text style={styles.empty}>No self care logged.</Text>
          )}
      ...
      <Modal visible={noteModal} transparent animationType="slide">
        <View style={styles.modalWrap}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Self Care Reflection</Text>

            <TextInput
              placeholder="How did this help you? How do you feel?"
              value={note}
