...
const DEFAULT_PRIVACY = [
  {
    ...
    color: '#FDE68A',
    items: [
      { id: 'd1', text: 'Reviewed Account Permissions' },
      { id: 'd2', text: 'Removed Unused Accounts' },
      { id: 'd3', text: 'Enabled Two-Factor Authentication' }
    ]
  },
  {
    id: 'security',
    ...
    color: '#BBF7D0',
    items: [
      { id: 's1', text: 'Updated Passwords' },
      { id: 's2', text: 'Reviewed Privacy Settings' },
      { id: 's3', text: 'Checked Device Security' }
    ]
  },
  {
    id: 'boundaries',
    ...
    color: '#FCA5A5',
    items: [
      { id: 'b1', text: 'Unsubscribed from Spam' },
      { id: 'b2', text: 'Set Communication Limits' },
      { id: 'b3', text: 'Blocked Harmful Accounts' }
    ]
  },
  {
    id: 'data',
    ...
    color: '#C7D2FE',
    items: [
      { id: 'p1', text: 'Requested Data Deletion' },
      { id: 'p2', text: 'Reviewed Data Sharing Policies' },
      { id: 'p3', text: 'Checked App Permissions' }
    ]
  }
];
...
const PrivacyScreen = ({ userId = null, supabaseClient = null }) => {

  const [categories, setCategories] = useState(DEFAULT_PRIVACY);
  const [selectedCategory, setSelectedCategory] = useState(DEFAULT_PRIVACY[0]);
  ...
  const handleLog = async (item) => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('privacy_history').insert({
        user_id: userId,
        action: item.text,
        category: selectedCategory.id,
  ...
  const addAction = async () => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('custom_privacy').insert({
        user_id: userId,
        action: newAction,
        created_at: new Date().toISOString()
      });
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Protect your privacy & boundaries 🔐</Text>
        </View>
        ...
        {/* Search */}
        <TextInput
          placeholder="Search privacy actions..."
          value={search}
        ...
        <View style={styles.addBox}>
          <Text style={styles.addTitle}>Add New Privacy Action</Text>
          <TextInput
            placeholder="ex: Reviewed Instagram privacy settings"
            value={newAction}
        ...
        <View style={styles.journal}>
          <Text style={styles.journalTitle}>Weekly Privacy Journal</Text>
          ...
          {dayEntries.length === 0 && (
            <Text style={styles.empty}>No privacy actions logged.</Text>
          )}
      ...
      <Modal visible={noteModal} transparent animationType="slide">
        <View style={styles.modalWrap}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Privacy Reflection</Text>

            <TextInput
              placeholder="Why did this matter? How did this protect your privacy?"
              value={note}
