 ...
 * NetworkScreen
 * ---------------------------------------
 * - Professional / Career Relationship Builder
 * - Categories (tabs)
 * - Search actions
 * - Add custom networking actions
 * - Log → popup reflection note
 * - Weekly expandable calendar journal
...
const DEFAULT_NETWORK = [
  {
    ...
    color: '#BFDBFE',
    items: [
      { id: 'n1', text: 'Reached out to professional contact' },
      { id: 'n2', text: 'Scheduled a call / meeting' },
      { id: 'n3', text: 'Followed up on opportunity' }
    ]
  },
  {
    id: 'learning',
    ...
    color: '#FDE68A',
    items: [
      { id: 'l1', text: 'Connected with mentor' },
      { id: 'l2', text: 'Asked for advice / feedback' },
      { id: 'l3', text: 'Shared knowledge or resource' }
    ]
  },
  {
    id: 'visibility',
    ...
    color: '#C7D2FE',
    items: [
      { id: 'v1', text: 'Posted / Contributed professionally' },
      { id: 'v2', text: 'Engaged with a post' },
      { id: 'v3', text: 'Shared achievements / updates' }
    ]
  },
  {
    id: 'community',
    ...
    color: '#BBF7D0',
    items: [
      { id: 'c1', text: 'Attended networking event' },
      { id: 'c2', text: 'Joined discussion / group' },
      { id: 'c3', text: 'Met a new industry contact' }
    ]
  }
];
...
const NetworkScreen = ({ userId = null, supabaseClient = null }) => {

  const [categories, setCategories] = useState(DEFAULT_NETWORK);
  const [selectedCategory, setSelectedCategory] = useState(DEFAULT_NETWORK[0]);
  ...
  const handleLog = async (item) => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('network_history').insert({
        user_id: userId,
        action: item.text,
        category: selectedCategory.id,
  ...
  const addAction = async () => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('custom_network').insert({
        user_id: userId,
        action: newAction,
        created_at: new Date().toISOString()
      });
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Grow your opportunities ⭐</Text>
        </View>
        ...
        {/* Search */}
        <TextInput
          placeholder="Search networking actions..."
          value={search}
        ...
        <View style={styles.addBox}>
          <Text style={styles.addTitle}>Add New Networking Action</Text>
          <TextInput
            placeholder="ex: Sent LinkedIn message"
            value={newAction}
        ...
        <View style={styles.journal}>
          <Text style={styles.journalTitle}>Weekly Networking Journal</Text>
          ...
          {dayEntries.length === 0 && (
            <Text style={styles.empty}>No networking logged today.</Text>
          )}
      ...
      <Modal visible={noteModal} transparent animationType="slide">
        <View style={styles.modalWrap}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Networking Reflection</Text>

            <TextInput
              placeholder="How did this interaction go? What did it lead to?"
              value={note}
...
const styles = StyleSheet.create({
  ...
  addBtn:{ backgroundColor:'#2563EB', padding:10, borderRadius:10, alignItems:'center' },
  ...
  noteBtn:{ backgroundColor:'#2563EB', padding:8, borderRadius:8, alignItems:'center' },
  ...
  modalBtn:{ backgroundColor:'#2563EB', padding:10, borderRadius:10, marginTop:10 },
