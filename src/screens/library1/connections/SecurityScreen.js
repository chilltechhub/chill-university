...
const DEFAULT_SECURITY = [
  {
    ...
    color: '#FDE68A',
    items: [
      { id: 'p1', text: 'Stayed aware of surroundings' },
      { id: 'p2', text: 'Told someone where I was going' },
      { id: 'p3', text: 'Trusted instincts & avoided risk' }
    ]
  },
  {
    id: 'digital',
    ...
    color: '#BBF7D0',
    items: [
      { id: 'd1', text: 'Updated passwords' },
      { id: 'd2', text: 'Enabled 2FA' },
      { id: 'd3', text: 'Avoided risky links/apps' }
    ]
  },
  {
    id: 'financial',
    ...
    color: '#FCA5A5',
    items: [
      { id: 'f1', text: 'Reviewed bank or spending' },
      { id: 'f2', text: 'Avoided unnecessary risk' },
      { id: 'f3', text: 'Protected sensitive info' }
    ]
  },
  {
    id: 'environment',
    ...
    color: '#C7D2FE',
    items: [
      { id: 'e1', text: 'Locked doors / checked safety' },
      { id: 'e2', text: 'Prepared emergency item' },
      { id: 'e3', text: 'Created safer space' }
    ]
  }
];
...
const SecurityScreen = ({ userId = null, supabaseClient = null }) => {

  const [categories, setCategories] = useState(DEFAULT_SECURITY);
  const [selectedCategory, setSelectedCategory] = useState(DEFAULT_SECURITY[0]);
  ...
  const handleLog = async (item) => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('security_history').insert({
        user_id: userId,
        action: item.text,
        category: selectedCategory.id,
  ...
  const addAction = async () => {
    ...
    if (supabaseClient && userId) {
      await supabaseClient.from('custom_security').insert({
        user_id: userId,
        action: newAction,
        created_at: new Date().toISOString()
      });
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.sub}>Keep yourself safe & secure 🔐</Text>
        </View>
        ...
        {/* Search */}
        <TextInput
          placeholder="Search security actions..."
          value={search}
        ...
        <View style={styles.addBox}>
          <Text style={styles.addTitle}>Add New Security Action</Text>
          <TextInput
            placeholder="ex: Checked privacy settings"
            value={newAction}
        ...
        <View style={styles.journal}>
          <Text style={styles.journalTitle}>Weekly Security Log</Text>
          ...
          {dayEntries.length === 0 && (
            <Text style={styles.empty}>No security actions logged.</Text>
          )}
      ...
      <Modal visible={noteModal} transparent animationType="slide">
        <View style={styles.modalWrap}>
          <View style={styles.modalBox}>
            <Text style={styles.modalTitle}>Security Reflection</Text>

            <TextInput
              placeholder="Why did this matter? How did it help your safety?"
              value={note}
