// src/screens/library/wellness/ExerciseScreen.js
...
const CATEGORIES = ['Cardio', 'Strength', 'Flexibility', 'Sports', 'Walk'];
...
export default function ExerciseScreen() {
  ...
  const loadEntries = async (uid) => {
    ...
    } catch (e) { console.warn('ExerciseScreen', e); }
    setLoading(false);
  };
  ...
  const accentColor = c.error;

  return (
    <View style={{ flex:1, backgroundColor:c.bg0 }}>
      {/* Header */}
      <View style={{ backgroundColor:c.headerBg, padding:s.lg, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <Text style={{ fontSize:t.xxl, fontWeight:t.bold, color:c.text1 }}>💪 Exercise</Text>
        <Text style={{ fontSize:t.xs, color:c.text3, marginTop:4 }}>Track and reflect on your exercise habits</Text>
      </View>
      ...
      <View style={{ flexDirection:'row', gap:s.sm, paddingHorizontal:s.lg, paddingBottom:s.sm, backgroundColor:c.bg1, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <TextInput
          ...
          placeholder="Log an exercise or workout..." placeholderTextColor={c.text4}
