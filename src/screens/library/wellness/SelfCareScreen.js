// src/screens/library/wellness/SelfCareScreen.js
...
const CATEGORIES = ['Morning', 'Evening', 'Rest', 'Social', 'Creative'];
...
export default function SelfCareScreen() {
  ...
  const loadEntries = async (uid) => {
    ...
    try {
      const { data } = await supabase
        ...
        .select('*')
        .eq('user_id', uid)
        .eq('area_id', 'spiritual')
        .order('created_at', { ascending: false })
        .limit(30);
      if (data) setEntries(data);
    } catch (e) { console.warn('SelfCareScreen', e); }
    setLoading(false);
  };
  ...
  const accentColor = c.gold;

  return (
    <View style={{ flex:1, backgroundColor:c.bg0 }}>
      {/* Header */}
      <View style={{ backgroundColor:c.headerBg, padding:s.lg, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <Text style={{ fontSize:t.xxl, fontWeight:t.bold, color:c.text1 }}>✨ Self-care</Text>
        <Text style={{ fontSize:t.xs, color:c.text3, marginTop:4 }}>Track and reflect on your self-care habits</Text>
      </View>
      ...
      <View style={{ flexDirection:'row', gap:s.sm, paddingHorizontal:s.lg, paddingBottom:s.sm, backgroundColor:c.bg1, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <TextInput
          ...
          placeholder="Log a self-care activity..." placeholderTextColor={c.text4}
          ...
          ListEmptyComponent={
            <View style={{ alignItems:'center', paddingTop:60 }}>
              ...
              <Text style={{ fontSize:t.md, color:c.text3, textAlign:'center' }}>
                Log your first self-care to get started
              </Text>
            </View>
          }
          renderItem={({ item }) => (
