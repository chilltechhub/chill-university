...
export default function WellnessAreaScreen({
  ...
  // The action panel; what it logs lands straight in this screen's history.
  const aa = useAreaActions({
    screenTag, areaId, navigation,
  ...
  // The old Log view, unchanged, now inside the Details & history drawer.
  const renderLog = () => (
    <View style={{ gap: s.md }}>
      {/* Category filter */}
  ...
  const renderDeeper = () => (
    <>
      <GoDeeperList resources={aa.resources} color={accentColor} />
      <Text style={{ fontSize: t.xs, color: accentColor, textTransform: 'uppercase', letterSpacing: 1.2, fontWeight: t.bold, marginTop: s.xl, marginBottom: s.md }}>
        Linked in the app
      </Text>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.bg0 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: s.lg, paddingBottom: 60, gap: s.xl }} showsVerticalScrollIndicator={false}>
        ...
        <DetailsDrawer
          color={accentColor}
          summary="Links for going deeper, your log and history"
          initialTab="deeper"
          tabs={[
      ...
      <ActionEditSheet visible={editOpen} onClose={() => setEditOpen(false)} aa={aa} color={accentColor} title={title} />
    </View>
  );
}
