...
export default function ProjectDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const uid = auth.data?.user?.id || null; setUserId(uid); const journals = (journal.data || []).map(x => ({ ...x, kind: x.type === 'idea' ? 'ideas' : x.type === 'question' ? 'questions' : 'notes' })); const all = [...(tasks.data || []).map(x => ({ ...x, kind: 'tasks' })), ...journals, ...(research.data || []).map(x => ({ ...x, kind: 'research', body: x.notes }))].sort((a, b) => new Date(b.created_at) - new Date(a.created_at)); setObjects(all); setActivity([...(milestones.data || []), ...journals.filter(x => x.kind === 'notes')].sort((a,b) => new Date(b.created_at) - new Date(a.created_at))); setRawData({ tasks: tasks.data || [], journal: journal.data || [], research: research.data || [], milestones: milestones.data || [] }); setLoading(false);
