...
const STAGE_MILESTONE = { idea: '📐 Back to the blueprint', active: '🔨 Building', completed: '🚀 Shipped' };
...
export default function ProjectDetailScreen() {
  ...
  const setStage = async (status) => {
    ...
    if (error) {
      ...
      Alert.alert('Could not move this build', 'Something went wrong — try again.');
    } else {
      setProject(p => ({ ...p, status }));
      if (status === 'completed') signalAction('project-shipped');
      // The build log is where "I shipped this" gets remembered.
