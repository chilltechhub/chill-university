...
- 3.1 Home: done 2026-09-22 on `feature/scenario-gaps`. The top-ranked desk item shows as a NextUpCard with its actions, and the rest drift past in the ticker, which stops for Reduce Motion. Every persona now leads with Compass, then desk, then Commander, and so does the pre-dashboard starter Home. The desk arrives with the Capture Inbox stage. Saved layouts keep their own order.
- Found while tracing six user scenarios (same branch): nothing could set a project's stage, so it could never ship. ProjectDetail now has Blueprint/Building/Shipped chips that log milestones, and the ship/next-step goal steps tick themselves. Inbox "Start New Project" created nothing and dropped the capture; it now builds the project. Inbox → Planner asks for a day and an area. New-project sheets have a first-next-step field, and New Build has starter examples. Each type's missing core tool (Workshop/Vault) moved from stage 8 to 6. Check-ins Due joins with Life Areas.
