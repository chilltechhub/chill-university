...
export const generateDailyGoals = (subjects) => {
  ...
  selectedSubjects.forEach((subject) => {
    const templates = DAILY_GOAL_TEMPLATES[subject];
...
export const generateWeeklyMissions = (subjects) => {
  ...
  const selectedSubjects = subjects.slice(0, 2);
  selectedSubjects.forEach((subject) => {
    const templates = WEEKLY_MISSION_TEMPLATES[subject];
    const template = templates[Math.floor(Math.random() * templates.length)];
    missions.push({
...
export const updateGoalProgress = (goal, increment) => {
  const updated = { ...goal };
  updated.current = Math.min((updated.current || 0) + increment, updated.target);
...
// ==========================================
// INITIAL STATS (from old UserProgressContext)
// ==========================================
...
export function UserProgressProvider({ children }) {
  ...
  // Load stats from AsyncStorage and Supabase
  useEffect(() => {
    let mounted = true;
    ...
    const loadLocal = async () => {
      try {
        ...
        if (json && mounted) setStats(JSON.parse(json));
      } catch (e) {
  ...
  const completeQuestion = (subject, correct = false, points = 10) => {
    ...
    const newAccuracy =
      ((subjData.accuracy || 0) * (subjData.questionsAnswered || 0) + (correct ? 1 : 0)) /
      newQuestions;
    ...
    if (last) {
      const diffDays = Math.floor(
        (now.setHours(0, 0, 0, 0) - new Date(last).setHours(0, 0, 0, 0)) / (1000 * 60 * 60 * 24)
      );
    ...
    newStats.userProgress.longtermGoals = newStats.userProgress.longtermGoals.map((lt) => {
      if (lt.subject === subject) {
        ...
        if (lt.category === 'mastery') {
          const masteryPercent = Math.min(
            100,
            Math.round((subjData.questionsAnswered / (lt.tiers[lt.tiers.length - 1].target || 1)) * 100)
          );
          return updateLongtermGoal(lt, masteryPercent).goal;
        }
