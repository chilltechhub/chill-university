// src/screens/library/wellness/WellbeingScreen.js
...
const CATEGORIES = ['Mood', 'Stress', 'Sleep', 'Mindfulness', 'Gratitude'];
...
export default function WellbeingScreen() {
  ...
  const loadEntries = async (uid) => {
    ...
    } catch (e) { console.warn('WellbeingScreen', e); }
    setLoading(false);
  };
  ...
  const accentColor = c.purple;

  return (
    <View style={{ flex:1, backgroundColor:c.bg0 }}>
      {/* Header */}
      <View style={{ backgroundColor:c.headerBg, padding:s.lg, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <Text style={{ fontSize:t.xxl, fontWeight:t.bold, color:c.text1 }}>💙 Well-being</Text>
        <Text style={{ fontSize:t.xs, color:c.text3, marginTop:4 }}>Track and reflect on your feeling habits</Text>
      </View>
      ...
      <View style={{ flexDirection:'row', gap:s.sm, paddingHorizontal:s.lg, paddingBottom:s.sm, backgroundColor:c.bg1, borderBottomWidth:0.5, borderBottomColor:c.border }}>
        <TextInput
          ...
          placeholder="How are you feeling today?" placeholderTextColor={c.text4}
          ...
          ListEmptyComponent={
            <View style={{ alignItems:'center', paddingTop:60 }}>
              ...
              <Text style={{ fontSize:t.md, color:c.text3, textAlign:'center' }}>
                Log your first feeling to get started
              </Text>
            </View>
          }
          renderItem={({ item }) => (
